...
class Tracker:
    ...
    def add_position_to_tracks(self, tracks):
        """Add foot position to each player track for distance calculations"""
        for frame_num, player_dict in enumerate(tracks['players']):
            for track_id, track in player_dict.items():
                ...
                position = (position[0], int(bbox[3]))  # x_center, y_bottom
                tracks['players'][frame_num][track_id]['position'] = position
        return tracks

    def calculate_player_statistics(self, tracks, fps=24.0, pixel_to_meter=0.1):
        """
        Calculate distance traveled and speed for each player
        fps: frames per second of the video
        pixel_to_meter: conversion factor from pixels to meters
        """
        from utils.bbox_utils import measure_distance
        
        # Track each player's total distance and previous position
        player_stats = {}
        ...
        for frame_num, player_dict in enumerate(tracks['players']):
            for track_id, track in player_dict.items():
                ...
                if player_stats[track_id]['prev_position'] is not None:
                    pixel_distance = measure_distance(player_stats[track_id]['prev_position'], current_position)
                    distance_meters = pixel_distance * pixel_to_meter
                    player_stats[track_id]['total_distance'] += distance_meters
                    ...
                    # Smooth speed using moving average (last 5 frames)
                    player_stats[track_id]['speed_history'].append(speed_kmh)
                    if len(player_stats[track_id]['speed_history']) > 5:
                        player_stats[track_id]['speed_history'].pop(0)
                    
                    current_speed = np.mean(player_stats[track_id]['speed_history'])
                else:
                    current_speed = 0.0
                
                # Store in track
                tracks['players'][frame_num][track_id]['total_distance'] = player_stats[track_id]['total_distance']
                tracks['players'][frame_num][track_id]['speed'] = current_speed
                
                # Update previous position
                player_stats[track_id]['prev_position'] = current_position
        
        return tracks
    def draw_ellipse(self,frame,bbox,color,track_id=None, distance=None, speed=None):
        y2 = int(bbox[3])
        ...
        # Draw distance and speed below the ellipse
        text_y_offset = y2 + 35
        ...
        if distance is not None:
            ...
            cv2.putText(
                frame,
                distance_text,
                (int(x_center - 25), int(text_y_offset)),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 0, 0),  # Black shadow
                3
        ...
        if speed is not None:
            ...
            cv2.putText(
                frame,
                speed_text,
                (int(x_center - 30), int(text_y_offset)),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 0, 0),  # Black shadow
                3
    ...
    def draw_annotations(self,video_frames, tracks, team_ball_control):
            ...
            for frame_num, frame in enumerate(video_frames):
                ...
                for track_id, player in player_dict.items():
                    ...
                    frame = self.draw_ellipse(frame, player["bbox"], color, track_id, distance, speed)
